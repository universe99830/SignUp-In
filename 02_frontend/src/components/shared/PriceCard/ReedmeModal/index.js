...
const {open,onClose,showQRWindow,balance,level,data,hasNFT} = props
...
const offer = data.OutSyde_offer
...
const handleClick = async(ele)=>{
    ...
    if(level == 3 && !hasNFT){
        toast.error("You have not OutSyde NFT.")
        swipebtn.current.reset()
        return
    }
    var value = val;
